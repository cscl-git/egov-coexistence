...
public class LegalCaseService extends PersistenceService<LegalCase, Long>{	
	@Autowired
    ...
    public LegalCaseService(final Class<LegalCase> type) {
        super(type);
    }
    ...
    @Transactional
    public LegalCase persist(final LegalCase legalcase, final List<AttachedDocument> files)
        ...
        if (legalcase != null) {
            ...
            for (final BipartisanDetails petitioner : legalcase.getBipartisanPetitionerDetailsList()) {
                if (petitioner.getName() != null && !petitioner.getName().trim().isEmpty()) {
            		...
            		if (petitioner.isNew()) {
            			petitioner.setCreatedBy(ApplicationThreadLocals.getUserId());
            			petitioner.setCreatedDate(currentDate);
            		}
            		petitioner.setLastModifiedBy(ApplicationThreadLocals.getUserId());
            		petitioner.setLastModifiedDate(currentDate);
...
//                        respondent.setIsRespondentGovernment(Boolean.FALSE);
        ...
        if (!legalcase.getPwrList().isEmpty()) {
            for (final Pwr legalpwr : legalcase.getPwrList()) {
                legalpwr.setLegalCase(legalcase);
                if (legalpwr.isNew()) {
                	legalpwr.setCreatedBy(ApplicationThreadLocals.getUserId());
                	legalpwr.setCreatedDate(currentDate);
        		}
                legalpwr.setLastModifiedBy(ApplicationThreadLocals.getUserId());
                legalpwr.setLastModifiedDate(currentDate);
                pwrListtemp.add(legalpwr);
            }
            legalcase.getPwrList().clear();
            legalcase.setPwrList(pwrListtemp);
    ...
    public List<LegalCaseUploadDocuments> getLegalcaseUploadDocumentDetails(final LegalCase legalCase,
    		final List<AttachedDocument> files) throws IOException {
