...
import org.egov.egf.model.BillRegisterNewReportBean;
...
public class CreateJournalVoucherController extends BaseVoucherController {
	...
	private static final String JOURNALVOUCHER_SEARCH_NEW = "journalvoucher-searchnew";
	...
	private static Map<String, List<String>> netAccountCode = new HashMap<String, List<String>>(); // have list of all
																									// net payable
	@Autowired
	...
	@Autowired
	private VouchermisService vmis;
	...
	public String create(@ModelAttribute("voucherHeader") final CVoucherHeader voucherHeader, final Model model,
		...
		if (resultBinder.hasErrors()) {
			...
			try {
				savedVoucherHeader = journalVoucherService.create(voucherHeader, approvalPosition, approvalComment,
						null, workFlowAction);
	...
	public String searchVoucher(@ModelAttribute("voucherHeader") final CVoucherHeader voucherHeader, final Model model,
			final HttpServletRequest request) {
		voucherHeader.setType(FinancialConstants.STANDARD_VOUCHER_TYPE_JOURNAL);
		model.addAttribute(STATE_TYPE, voucherHeader.getClass().getSimpleName());
		prepareWorkflow(model, voucherHeader, new WorkflowContainer());
	...
	@RequestMapping(value = "/newsearchVoucher", method = RequestMethod.POST)
	public String newSearchVoucher(@ModelAttribute("voucherHeader") final CVoucherHeader voucherHeader, final Model model,
			final HttpServletRequest request) {
		voucherHeader.setType(FinancialConstants.STANDARD_VOUCHER_TYPE_JOURNAL);
		model.addAttribute(STATE_TYPE, voucherHeader.getClass().getSimpleName());
		prepareWorkflow(model, voucherHeader, new WorkflowContainer());
	...
	public String searchNewVoucherResult(@ModelAttribute("voucherHeader") final CVoucherHeader voucherHeader,
		...
		query1.append(
				...
				+ "	eb3.accountdetailkeyid = a.detailkey\r\n"
	...
	@RequestMapping(value = "/searchNewVoucherResult", params = "export", method = RequestMethod.POST)
	public @ResponseBody ResponseEntity<InputStreamResource> newVoucherExport(
			@ModelAttribute("voucherHeader") final CVoucherHeader voucherHeader, final Model model,
	...
	public String getNewVoucherDateQuery(final Date billDateFrom, final Date billDateTo) {
		final StringBuffer numDateQuery = new StringBuffer();
		try {

			if (null != billDateFrom)
				numDateQuery.append(" and eb.billdate>='").append(DDMMYYYYFORMAT1.format(billDateFrom)).append("'");
			if (null != billDateTo)
				numDateQuery.append(" and eb.billdate<='").append(DDMMYYYYFORMAT1.format(billDateTo)).append("'");
	...
	public String getNewVoucherMisQuery(final CVoucherHeader voucherHeader) {
		...
		if (null != voucherHeader) {
			if (voucherHeader.getVouchermis() != null && voucherHeader.getVouchermis().getDepartmentcode() != null
					&& !voucherHeader.getVouchermis().getDepartmentcode().isEmpty()) {
				misQuery.append(" and ed.\"name\"='")
	...
	public String searchVoucherResult(@ModelAttribute("voucherHeader") final CVoucherHeader voucherHeader,
			final Model model, final HttpServletRequest request) {
		...
		List<Object[]> list = null;
		query1.append(
				"select vdm.voucherid ,vdm.vouchernumber ,vdm.status,vdm.head,vdm.department,vdm.voucherdate,vdm.scheme from voucher_detail_main vdm where vdm.fund ="
						+ voucherHeader.getFundId().getId())
		...
		list = null;
		query2.append("select vdb.id,vdb.vouchernumber,vdb.voucherdate from voucher_detail_bpvmapping vdb");
		...
		list = null;
		query3.append("select vdm.id,vdm.billvhid,vdm.payvhid,vdm.paidamount from voucher_detail_miscbill vdm ");
		...
		list = null;
		query4.append("select vdp.id,vdp.detailname from voucher_detail_party vdp  ");
		...
		list = null;
		query7.append(
				"select vdl.glcode,vdl.creditamount,vdl.debitamount,vdl.voucherheaderid from voucher_detail_ledger  vdl  ");
		...
		list = null;
		query10.append(
				"select t.id,ch.glcode from tds t, chartofaccounts ch where t.glcodeid = ch.id and isactive =true");
		...
		list = null;
		query9.append("select vdm.payvhid , vdm.paidamount from voucher_detail_misc vdm  ");
			...
			if (voucherDetailMiscMapping.get(key) != null) {
				for (VoucherDetailMiscMapping row : voucherDetailMiscMapping.get(key)) {
					...
					if (row.getBpvId() != null) {
						resultset.setPaymentVoucherNumber(
								voucherDetailBpvMapping.get(row.getBpvId()).getVoucherNumber());
						...
						if (voucherDetailIntrumentMapping.get(row.getBpvId()) != null
								&& voucherDetailIntrumentMapping.get(row.getBpvId()).getVoucherNumber() != null
								&& !voucherDetailIntrumentMapping.get(row.getBpvId()).getVoucherNumber().isEmpty()) {
							resultset.setPexNo(voucherDetailIntrumentMapping.get(row.getBpvId()).getVoucherNumber());
						}
						if (voucherDetailIntrumentMapping.get(row.getBpvId()) != null
								&& voucherDetailIntrumentMapping.get(row.getBpvId()).getApprovalComent() != null
						...
						if (voucherDetailIntrumentMapping.get(row.getBpvId()) != null
								...
								&& !voucherDetailIntrumentMapping.get(row.getBpvId()).getFirstsignatory().isEmpty()) {
							resultset.setChqNo(voucherDetailIntrumentMapping.get(row.getBpvId()).getFirstsignatory());
						}
						if (voucherDetailIntrumentMapping.get(row.getBpvId()) != null
								&& voucherDetailIntrumentMapping.get(row.getBpvId()).getSecondsignatory() != null
								&& !voucherDetailIntrumentMapping.get(row.getBpvId()).getSecondsignatory().isEmpty()) {
							resultset
									.setChqDate(voucherDetailIntrumentMapping.get(row.getBpvId()).getSecondsignatory());
						}
						//
						if (voucherDetailIntrumentMapping.get(row.getBpvId()) != null
								&& voucherDetailIntrumentMapping.get(row.getBpvId()).getCgvn() != null
								&& !voucherDetailIntrumentMapping.get(row.getBpvId()).getCgvn().isEmpty()) {
					...
					try {
						Vouchermis vmis_receipt_no = vmis.getVouchermisByVoucherId(key);
						resultset.setReceiptNo(vmis_receipt_no.getRecieptNumber());
					} catch (Exception e) {
				...
				if (voucherMiscBillMapping.get(result.getId()) != null
						&& voucherMiscBillMapping.get(result.getId()).getVoucherNumber() != null
						&& !((voucherMiscBillMapping.get(result.getId()).getVoucherNumber()).isEmpty())) {
					resultset.setPaidAmount(
							new BigDecimal(voucherMiscBillMapping.get(result.getId()).getVoucherNumber()));
				}
				resultset.setDepartmentCode(result.getDepartment());
				resultset.setBudgetHead(result.getHead());
				resultset.setVoucherNumber(result.getVoucherNumber());
				...
				if (voucherDetailIntrumentMapping.get(result.getId()) != null
						&& voucherDetailIntrumentMapping.get(result.getId()).getVoucherNumber() != null
						&& !voucherDetailIntrumentMapping.get(result.getId()).getVoucherNumber().isEmpty()) {
				...
				// checque
				if (voucherDetailIntrumentMapping.get(result.getId()) != null
						&& voucherDetailIntrumentMapping.get(result.getId()).getFirstsignatory() != null
						&& !voucherDetailIntrumentMapping.get(result.getId()).getFirstsignatory().isEmpty()) {
					resultset.setChqNo(voucherDetailIntrumentMapping.get(result.getId()).getFirstsignatory());
				}
				if (voucherDetailIntrumentMapping.get(result.getId()) != null
						&& voucherDetailIntrumentMapping.get(result.getId()).getSecondsignatory() != null
						&& !voucherDetailIntrumentMapping.get(result.getId()).getSecondsignatory().isEmpty()) {
					resultset.setChqDate(voucherDetailIntrumentMapping.get(result.getId()).getSecondsignatory());
				}
				//
				if (voucherDetailIntrumentMapping.get(result.getId()) != null
						&& voucherDetailIntrumentMapping.get(result.getId()).getCgvn() != null
						&& !voucherDetailIntrumentMapping.get(result.getId()).getCgvn().isEmpty()) {
	...
	private void populateTax(BillRegisterReportBean resultset, VoucherDetailMain result,
			Map<Long, List<VoucherDetailLedger>> voucherDetailLedgerInstrument, List<String> tds) {
		...
		BigDecimal electricityChargesAmount = new BigDecimal("0");
		BigDecimal adminServiceChargesAmount = new BigDecimal("0");
		BigDecimal otherChargesAmount = new BigDecimal("0");
		BigDecimal otherRecovFromEmployeesAmount = new BigDecimal("0");
		...
		if (detail != null && !detail.isEmpty()) {
			...
			resultset.setElectricityChargesAmount(electricityChargesAmount);
			resultset.setAdminServiceChargesAmount(adminServiceChargesAmount);
			resultset.setOtherChargesAmount(otherChargesAmount);
			resultset.setOtherRecovFromEmployeesAmount(otherRecovFromEmployeesAmount);
			int i = 0;
			...
			decTax = decTax.add(tdsTax).add(igstTax).add(cgstTax).add(laborTax).add(colTax).add(waterTax)
					.add(qualCessTax).add(fineTax).add(secTax).add(gpfAmount).add(npsAmount).add(gslicAmount)
					.add(hbaAmount).add(licenseAmount).add(licAmount).add(bankAmount).add(courtAmount)
					.add(pensionAmount).add(electricityChargesAmount).add(adminServiceChargesAmount)
					.add(otherChargesAmount).add(otherRecovFromEmployeesAmount).add(anyTax);
	...
	public @ResponseBody ResponseEntity<InputStreamResource> voucherExport(
			@ModelAttribute("voucherHeader") final CVoucherHeader voucherHeader, final Model model,
			final HttpServletRequest request) throws IOException {
		LOGGER.info("export");
		List<BillRegisterReportBean> billRegReportList = new ArrayList<BillRegisterReportBean>();
		...
		String[] COLUMNS = { "S.no.", "Party Name", "DIVISION", "BUDGET HEAD", "Scheme", "Gross Amount", "TDS/I",
				"TDS ON IGST", "TDS ON CGST/UTGST", "Labour Cess", "Collection charges", "Water charges",
				"Quality Cess", "Penalty/Fine", "Security/Amt withheld", "GPF", "NPS/CPF", "GSLIC/GIS",
				"House Building Advance", "Licence Fees", "LIC", "Bank Loan", "Court Attachment", "Pension Fund",
				"Electricity charges", "Admin/Service Charges", "Other Charges", "Other Recoveries From Employees",
				"Any other deduction", "Net Amount", "Paid Amount", "Journal Voucher number", "Jounal Voucher Date",
				"Payment voucher number", "BPV Date", "PEX NUMBER", "PEX DATE", "CHEQUE NUMBER", "CHEQUE DATE",
	...
	protected StringBuffer getQuery(CVoucherHeader voucherHeader) {
		...
		if (null != voucherHeader.getBillFrom())
			whereQuery.append(" and vh.voucherdate >='").append(DDMMYYYYFORMAT1.format(voucherHeader.getBillFrom()))
					.append("'");
		if (null != voucherHeader.getBillTo())
			whereQuery.append(" and vh.voucherdate <='").append(DDMMYYYYFORMAT1.format(voucherHeader.getBillTo()))
					.append("'");
	...
	protected String getQueryByExpndType(final String expndType, final String whereQuery,
		...
		query.append(" select  vh.vouchernumber, mis.departmentcode, vh.id as voucherid, mis.functionid")
				...
				.append(" where vh.type='Journal Voucher'   and mis.voucherheaderid =vh.id ").append(voucherQry)
	...
	private BigDecimal getnetAmount(Long voucherheaderid, BigDecimal grossamt, List<Object[]> rows,
			List<Object[]> otherDeductionAmtrows) {
		// SQLQuery query = null;
				...
				for (Object[] element : list) {
					...
					if (element[1] != null) {
						...
						if (!billDetail.getConsumerCode().equalsIgnoreCase("")) {
							if (
							/* tax codes */
							billDetail.getConsumerCode().equals("3502007")
									|| billDetail.getConsumerCode().equals("3502009")
									|| billDetail.getConsumerCode().equals("3502010")
									|| billDetail.getConsumerCode().equals("3502011")
									|| billDetail.getConsumerCode().equals("3502012") ||
									/* other glcodes */
									billDetail.getConsumerCode().equals("3502055")
									|| billDetail.getConsumerCode().equals("3502054")
									|| billDetail.getConsumerCode().equals("3502018")
									|| billDetail.getConsumerCode().equals("1408055")
									|| billDetail.getConsumerCode().equals("1405014")
									|| billDetail.getConsumerCode().equals("3502058")
									|| billDetail.getConsumerCode().equals("1402003")
									|| billDetail.getConsumerCode().equals("3401004")) {

								totaltax = totaltax.add(null != element[2]
										? new BigDecimal(element[2].toString()).setScale(2, BigDecimal.ROUND_HALF_EVEN)
	...
	private BigDecimal getgrossAmount(Long voucherheaderid, List<Object[]> rows) {
		...
		try {
			...
			for (Object[] element : list) {
				if (element[1] != null) {
					grossamt = null != element[1]
							? new BigDecimal(element[1].toString()).setScale(2, BigDecimal.ROUND_HALF_EVEN)
							: BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_EVEN);
				}
			}
	...
	private List<BillDetail> getbillDetails(Long billid, BigDecimal netAmount, List<Object[]> rows,
			List<Object[]> otherDedictionAmtrows) {
		String deducvh = "";
			...
			if (rows != null && !rows.isEmpty()) {
				...
				for (Object[] element : list) {
					...
					if (element[1] != null) {
						...
						if (!billDetail.getConsumerCode().equalsIgnoreCase("")) {
							if (billDetail.getConsumerCode().equals("3502007")
									|| billDetail.getConsumerCode().equals("3502009")
									|| billDetail.getConsumerCode().equals("3502010")
									|| billDetail.getConsumerCode().equals("3502011")
									|| billDetail.getConsumerCode().equals("3502012")) {
					...
					if (element[2] != null) {
						...
						if (!billDetail.getConsumerCode().equalsIgnoreCase("")) {
							if (!(billDetail.getConsumerCode().equals("3502007")
								...
								billDetail.setAmountPaid(null != element[2]
										? new BigDecimal(element[2].toString()).setScale(2, BigDecimal.ROUND_HALF_EVEN)
										: BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_EVEN));
							}
	...
	private Long getPayId(String voucherNumber) {
		...
		try {
			query = this.persistenceService.getSession().createSQLQuery(
					"select id,payvhid,billnumber from miscbilldetail m where m.billvhid in (select v.id from voucherheader v where v.vouchernumber =:vouchernumber)");
		...
		try {
			if (rows != null && !rows.isEmpty()) {
				...
				for (Object[] element : list) {
					...
					if (element[2] != null) {
						deducvh = element[2].toString();
						miscbilldetail.setPaidamount(null != element[2]
								? new BigDecimal(element[2].toString()).setScale(2, BigDecimal.ROUND_HALF_EVEN)
								: BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_EVEN));
					} else {
						deducvh = "";
	...
	private Long getVoucId(String voucherNumber) {
		...
		try {
			query = this.persistenceService.getSession().createSQLQuery(
					"select vh.id,vh.vouchernumber from voucherheader vh where vh.vouchernumber=:vouchernumber");
			query.setString("vouchernumber", voucherNumber);
			rows = query.list();
	...
	private String getMessageByStatus(final CVoucherHeader voucherHeader, final String approverName,
			final String nextDesign, final String workFlowAction) {
		String message;
	...
	public static ByteArrayInputStream resultToExcel(List<BillRegisterReportBean> billRegReportList, String[] COLUMNS)
		...
		for (BillRegisterReportBean detail : billRegReportList) {
			...
			if (detail.getElectricityChargesAmount() != null) {
				row.createCell(24).setCellValue(detail.getElectricityChargesAmount().doubleValue());
			}
			if (detail.getAdminServiceChargesAmount() != null) {
				row.createCell(25).setCellValue(detail.getAdminServiceChargesAmount().doubleValue());
			}
			if (detail.getOtherChargesAmount() != null) {
				row.createCell(26).setCellValue(detail.getOtherChargesAmount().doubleValue());
			}
			if (detail.getOtherRecovFromEmployeesAmount() != null) {
				row.createCell(27).setCellValue(detail.getOtherRecovFromEmployeesAmount().doubleValue());
			}

			if (detail.getDeductionAmount() != null) {
				row.createCell(28).setCellValue(detail.getDeductionAmount().doubleValue());
			}
			if (detail.getNetAmount() != null) {
				row.createCell(29).setCellValue(detail.getNetAmount().doubleValue());
			}
			if (detail.getPaidAmount() != null) {
				row.createCell(30).setCellValue(detail.getPaidAmount().doubleValue());
			}
			if (detail.getVoucherNumber() != null) {
				row.createCell(31).setCellValue(detail.getVoucherNumber());
			}
			if (detail.getVoucherDate() != null) {
				row.createCell(32).setCellValue(detail.getVoucherDate());
			}
			if (detail.getPaymentVoucherNumber() != null) {
				row.createCell(33).setCellValue(detail.getPaymentVoucherNumber());
			}
			if (detail.getBpvDate() != null) {
				row.createCell(34).setCellValue(detail.getBpvDate());
			}
			if (detail.getPexNo() != null) {
				row.createCell(35).setCellValue(detail.getPexNo());
			}
			if (detail.getPexNodate() != null) {
				row.createCell(36).setCellValue(detail.getPexNodate());
			}
			if (detail.getChqNo() != null) {
				row.createCell(37).setCellValue(detail.getChqNo());
			}
			if (detail.getChqDate() != null) {
				row.createCell(38).setCellValue(detail.getChqDate());
			}
			if (detail.getBankaccount() != null) {
				row.createCell(39).setCellValue(detail.getBankaccount());
			}
			if (detail.getStatus() != null) {
				row.createCell(40).setCellValue(detail.getStatus());
			}
		}
	...
	public static ByteArrayInputStream newResultToExcel(List<BillRegisterNewReportBean> billRegReportList, String[] COLUMNS)
			throws IOException {
		...
		Sheet sheet = workbook.createSheet("Voucher Detail New Report");
