...
@ParentPackage("egov")
@Results({
		@Result(name = BillRegisterSearchAction.NEW, location = "billRegisterSearch-" + BillRegisterSearchAction.NEW
				+ ".jsp"),
		@Result(name = BillRegisterSearchAction.POST, location = "billRegisterSearch-" + BillRegisterSearchAction.POST
				+ ".jsp") })
public class BillRegisterSearchAction extends BaseFormAction {
	...
	public String search() {
		...
		for (DateValidations dateValidations : billregisterDateValidate) {
			...
			if(ApplicationConstant.BILL_REGISTER_DATE_VALIDATION.equals(code)) {					
			...
			for (DateValidateByUser dateValidations2 : dateValidateByUser) {
				...
				Integer validDay = dateValidations2.getValidDay();
		         ...
		         String fromDateStr = billDateFrom;  // Starting date
		         String toDateStr = billDateTo;    // Ending date
		...
		query1.append("select bp.billnumber ,bp.detailname from bill_party bp");
		...
		query.append(
				...
				.append(" from EgBillregister br, EgBillregistermis billmis , EgwStatus egwstatus"
						+ " where   billmis.egBillregister.id = br.id and egwstatus.id = br.status.id  ")
		...
		sb.append(
				"select ad.egBillregister.id , ews.ownerName from AuditDetails ad, State ews where ews.id = ad.state.id ");
		final List<Object[]> listAuditDetails = (List<Object[]>) persistenceService.finds(sb.toString());

		final List<Object[]> list = persistenceService.findAllBy(query.toString(), expType);
		Map map = new HashMap<>();
		for (Object[] ll : listAuditDetails) {
			map.put(ll[0], ll[1]);
		}
		...
		if (stateIds != null && stateIds.size() > 0)
			...
			for (final Object[] owner : oWnerNamesList) {
				...
				try {
						System.out.println("::::: 1------- " + getLongValue(owner[0]));
						System.out.println("##### 2------- " + getLongValue(owner[1]));				
						String emp = null;
					if (owner[1] != null) {
						if (this.getEmployeeName(getLongValue(owner[1])) != null) {
							emp = this.getEmployeeName(getLongValue(owner[1]));
						}else {
							emp="";
						}
						System.out.println("$$$$  3------- " + emp);
						stateIdAndOwnerNameMap.put(getLongValue(owner[0]), emp);
					} else {
						emp = "";
						stateIdAndOwnerNameMap.put(getLongValue(owner[0]), emp);
			}
		...
		if (list.size() != 0) {
			...
			for (final Object[] object : list) {
				...
				if (object[6].toString().equals("Pending with Audit")) {
					billMap.put("billstatus", "Audit Task Created");
				} else {
				...
				if (null != object[7])
					...
					billMap.put("sourcepath", "/services/EGF/expensebill/view/" + object[8].toString());
	...
	private boolean populateParty(Map<String, List<VoucherDetailMain>> voucherDetailMiscMapping, String partyName2,
			String partyName3) {
		boolean status = false;
	...
	private List<Object[]> getOwnersForWorkFlowState(final List<Long> stateIds) {
		List<Object[]> ownerNamesList = new ArrayList<Object[]>();
		final String ownerNamesQueryStr = "select es.id,es.ownerPosition, es.ownerName from State es where es.id in (:IDS)";
		...
		if (size > 999) {
			...
			ownerNamesList = persistenceService.getSession().createQuery(ownerNamesQueryStr)
					.setParameterList("IDS", stateIds).list();
		return ownerNamesList;
	}

	public EgwStatus getStatusId(final String moduleType, final Integer statusid) {
		final String statusQury = "from EgwStatus where upper(moduletype)=upper('" + moduleType + "') and  id="
				+ statusid;
