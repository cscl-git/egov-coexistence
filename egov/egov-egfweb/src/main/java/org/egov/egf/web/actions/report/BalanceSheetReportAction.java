...
@ParentPackage("egov")
@Results({ @Result(name = "allScheduleDetailedResults", location = "balanceSheetReport-allScheduleDetailedResults.jsp"),
		...
		@Result(name = "balanceSheet-PDF", type = "stream", location = Constants.INPUT_STREAM, params = {
				Constants.INPUT_NAME, Constants.INPUT_STREAM, Constants.CONTENT_TYPE, "application/pdf",
				Constants.CONTENT_DISPOSITION, "no-cache;filename=BalanceSheet.pdf" }),
		@Result(name = "balanceSheet-XLS", type = "stream", location = Constants.INPUT_STREAM, params = {
				Constants.INPUT_NAME, Constants.INPUT_STREAM, Constants.CONTENT_TYPE, "application/xls",
				Constants.CONTENT_DISPOSITION, "no-cache;filename=BalanceSheet.xls" }),
		@Result(name = "balanceSheet-HTML", type = "stream", location = Constants.INPUT_STREAM, params = {
...
public class BalanceSheetReportAction extends BaseFormAction {
	...
	public void prepare() {
		...
		if (!parameters.containsKey("showDropDown")) {
			...
			// getPersistenceService().findAllBy("from CFinancialYear where isActive=true
			// and isActiveForPosting=true order by finYearRange desc "));
			addDropdownData("financialYearList",
	...
	protected void setRelatedEntitesOn() {
		...
		if (balanceSheet.getFinancialYear() != null && balanceSheet.getFinancialYear().getId() != null)
			balanceSheet.setFinancialYear((CFinancialYear) getPersistenceService()
		...
		/*
		 * if (balanceSheet.getField() != null && balanceSheet.getField().getId() !=
		 * null && balanceSheet.getField().getId() != 0) {
		 * balanceSheet.setField((Boundary)
		 * getPersistenceService().find("from Boundary where id=?",
		 * balanceSheet.getField().getId())); header.append(" in " +
		 * balanceSheet.getField().getName()); }
		 */
		if (balanceSheet.getFund() != null && balanceSheet.getFund().getId() != null
				&& balanceSheet.getFund().getId() != 0) {
			balanceSheet.setFund(
					(Fund) getPersistenceService().find("from Fund where id=?", balanceSheet.getFund().getId()));
		 ...
		 * balanceSheet.setFunctionary((Functionary)
		 * getPersistenceService().find("from Functionary where id=?",
		 * balanceSheet.getFunctionary().getId())); header.append(" in " +
		 * balanceSheet.getFunctionary().getName()); }
		 */
	...
	public String generateBalanceSheetPdf() throws Exception {
		...
		final JasperPrint jasper = reportHelper.generateFinancialStatementReportJasperPrint(balanceSheet,
				getText("report.heading"), header.toString(), getCurrentYearToDate(), getPreviousYearToDate(), true);
	...
	public String generateSchedulePdf() throws Exception {
		populateDataSourceForAllSchedules();
		final JasperPrint jasper = reportHelper.generateFinancialStatementReportJasperPrint(balanceSheet,
				getText("report.heading"), header.toString(), getCurrentYearToDate(), getPreviousYearToDate(), false);
		inputStream = reportHelper.exportPdf(inputStream, jasper);
		return BALANCE_SHEET_PDF;
	}
	...
	public String generateBalanceSheetSchedulePdf() throws Exception {
		populateDataSourceForSchedule();
		final JasperPrint jasper = reportHelper.generateFinancialStatementReportJasperPrint(balanceSheet,
				getText("report.sub.schedule.heading"), header.toString(), getCurrentYearToDate(),
	...
	private byte[] getBalanceSheetMinorAllScheduleResultsExcelSheet(Map<String, String> headerData,
