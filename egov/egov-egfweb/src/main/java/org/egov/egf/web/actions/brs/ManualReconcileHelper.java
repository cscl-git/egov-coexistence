...
public class ManualReconcileHelper {
	...
	public List<ReconcileBean> getUnReconciledCheques(ReconcileBean reconBean) 
	{
		...
		+ " case when rec.transactionType='Cr' then  'Payment' else 'Receipt' end as \"type\"  , upper(insType.type) as instrumentType FROM BANKRECONCILIATION rec, BANKACCOUNT BANK,"
		...
		" to_char(ih.transactiondate,'dd/mm/yyyy') as \"chequedate\" ,ih.instrumentAmount as \"chequeamount\",rec.transactiontype as \"txnType\", case when rec.transactionType= 'Cr' then 'Payment' else 'Receipt' end    as \"type\" , upper(insType.type) as instrumentType FROM BANKRECONCILIATION rec, BANKACCOUNT BANK,"
