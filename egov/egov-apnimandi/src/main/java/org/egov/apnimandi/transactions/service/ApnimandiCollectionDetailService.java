...
public class ApnimandiCollectionDetailService extends PersistenceService<ApnimandiCollectionDetails, Long> {
	...
	@Transactional
	public ApnimandiCollectionDetails persist(final ApnimandiCollectionDetails apnimandiCollectionDetails,
			final List<AttachedDocument> files, final List<ApnimandiCollectionAmountDetails> collectionAmountDetails,
			Long approvalPosition, String approvalComment, String workFlowAction) throws IOException, ParseException {
		if (approvalPosition != null && StringUtils.isNotEmpty(workFlowAction)) {
			apnimandiCollectionWorkflowCustomImpl.createCommonWorkflowTransition(apnimandiCollectionDetails,
		...
		final List<ApnimandiCollectionDocument> apnimandiCollectionDocuments = getCollectionDocumentDetails(
				apnimandiCollectionDetails, files);
		ApnimandiCollectionDetails savedApnimandiCollection = apnimandiCollectionDetailsRepository
				.save(apnimandiCollectionDetails);
		...
		if (null != workFlowAction) {
			if (ApnimandiConstants.APPROVE.equalsIgnoreCase(workFlowAction)) {
				...
				receiptResponse = receiptHeaderService.populateAndPersistReceipts(
						getReceiptHeader(savedApnimandiCollection), getReceiptInstrumentList(savedApnimandiCollection));
				String receiptNumber = receiptResponse.getReceipts().get(0).getBill().get(0).getBillDetails().get(0)
						.getReceiptNumber();
	...
	private ReceiptHeader getReceiptHeader(final ApnimandiCollectionDetails apnimandiCollectionDetails) {
		...
		receiptHeader.setService(
				ApnimandiConstants.SERVICE_PREFIX + "_" + apnimandiCollectionDetails.getZone().getRoadDivision() + "."
						+ apnimandiCollectionDetails.getServiceCategory());
		...
		for (ApnimandiCollectionAmountDetails amountDetails : apnimandiCollectionDetails
				.getApnimandiCollectionAmountDetails()) {
		...
		if (CollectionConstants.INSTRUMENTTYPE_CASH.equalsIgnoreCase(apnimandiCollectionDetails.getPaymentMode())) {
			...
			if (FinancialConstants.INSTRUMENT_TYPE_CHEQUE
					.equalsIgnoreCase(apnimandiCollectionDetails.getPaymentMode())) {
				instrumentHeader.setInstrumentType(
						financialsUtil.getInstrumentTypeByType(FinancialConstants.INSTRUMENT_TYPE_CHEQUE));
			} else {
				instrumentHeader.setInstrumentType(
						financialsUtil.getInstrumentTypeByType(FinancialConstants.INSTRUMENT_TYPE_DD));
			}
			instrumentHeader.setIsPayCheque(CollectionConstants.ZERO_INT);
			receiptHeader.getReceiptInstrument().add(instrumentHeader);
		}
	...
	private List<InstrumentHeader> getReceiptInstrumentList(
			final ApnimandiCollectionDetails apnimandiCollectionDetails) {
	...
	public List<ApnimandiCollectionDocument> getCollectionDocumentDetails(
			final ApnimandiCollectionDetails apnimandiCollectionDetails, final List<AttachedDocument> files)
			throws IOException {
		...
		if (null != files) {
			for (AttachedDocument attachedDocument : files) {
				...
				apnimandiCollectionDocument.setFilestoreid(
	...
	private void prepareCollectionAmounts(final ApnimandiCollectionDetails apnimandiCollectionDetails,
			final List<ApnimandiCollectionAmountDetails> collectionAmountDetails) {
		if (null != collectionAmountDetails) {
			if (!collectionAmountDetails.isEmpty()) {
				if (apnimandiCollectionDetails.getApnimandiCollectionAmountDetails() != null
						&& !apnimandiCollectionDetails.getApnimandiCollectionAmountDetails().isEmpty()) {
	...
	public String getReceipt(String selectedReceiptId) {
		...
		List<Receipt> receiptlist = microserviceUtils.searchReciepts(null, null, null, null,
				Arrays.asList(selectedReceiptId));
		ReceiptHeader receiptHeader = new ReceiptHeader();
			...
			receipt.getBill().forEach(bill -> {
				bill.getBillDetails().forEach(billDetail -> {
					...
					instrumentHeader.setInstrumentNumber(
							_instrument.getInstrumentNumber() != null ? _instrument.getInstrumentNumber()
									: _instrument.getTransactionNumber());
					instrumentHeader.setInstrumentDate(new Date(
							_instrument.getTransactionDateInput() != null ? _instrument.getTransactionDateInput()
									: _instrument.getInstrumentDate()));
					...
					EmployeeInfo empInfo = microserviceUtils
	...
	public List<ApnimandiCollectionSearchResult> getAllExistedCollections(
			final ApnimandiCollectionDetails apnimandiCollectionDetails) {
		...
		queryStr.append(
				"select distinct collectionObj as apnimandiCollections, zoneMaster.name as zoneName, egwStatus.description as statusName");
		queryStr.append(
				" from ApnimandiCollectionDetails collectionObj,EgwStatus egwStatus,ZoneMaster zoneMaster,ApnimandiContractor conObj");
		queryStr.append(
				" where collectionObj.status.id=egwStatus.id and collectionObj.zone.id=zoneMaster.id and collectionObj.contractor.id=conObj.id");
	...
	public List<ApnimandiCollectionSearchResult> search(final ApnimandiCollectionDetails apnimandiCollectionDetails) {
		final StringBuilder queryStr = new StringBuilder();
		queryStr.append(
				"select distinct collectionObj as apnimandiCollections, zoneMaster.name as zoneName, siteMaster.name as siteName, egwStatus.description as statusName");
		queryStr.append(
				" from ApnimandiCollectionDetails collectionObj,EgwStatus egwStatus,ZoneMaster zoneMaster, SiteMaster siteMaster, ApnimandiCollectionType collType");
		queryStr.append(
				" where collectionObj.status.id=egwStatus.id and collectionObj.zone.id=zoneMaster.id and collectionObj.site.id=siteMaster.id and collectionObj.collectiontype.id=collType.id");
	...
	private Query setParametersToQuery(final ApnimandiCollectionDetails apnimandiCollectionDetails,
	...
	private void getAppendQuery(final ApnimandiCollectionDetails apnimandiCollectionDetails,
	...
	public List<ApnimandiCollectionSearchResult> getCollectionsByContractor(
			final ApnimandiCollectionDetails apnimandiCollectionDetails) {
		if (null == apnimandiCollectionDetails.getCollectionForMonth()
				|| null == apnimandiCollectionDetails.getCollectionForYear()
		...
		queryStr.append(
				"select distinct collectionObj as apnimandiCollections, zoneMaster.name as zoneName, egwStatus.description as statusName");
		queryStr.append(
				" from ApnimandiCollectionDetails collectionObj,EgwStatus egwStatus,ZoneMaster zoneMaster,ApnimandiContractor apnimandiContractor");
		queryStr.append(
				" where collectionObj.status.id=egwStatus.id and collectionObj.zone.id=zoneMaster.id and collectionObj.contractor.id=apnimandiContractor.id");
		if (null != apnimandiCollectionDetails.getContractor()) {
	...
	public List<ApnimandiCollectionMISReport> dmCollectionReport(Date fromDate, Date toDate) {
		final StringBuilder queryStr = new StringBuilder();
		queryStr.append(
				"select distinct collectionObj.id as id, collectionObj.collectionDate as collectionDate, collectionObj.collectionForMonth as month, collectionObj.collectionForYear as year, collectionObj.serviceType as amountType, collectionObj.amount as totalAmount");
...
//		queryResult.setBoolean("active", Boolean.TRUE);
	...
	public List<EstimatedIncomeReport> estimatedIncomeReport(String collectionTypeCode, Date fromDate, Date toDate) {
		final StringBuilder queryStr = new StringBuilder();
		queryStr.append(
				" select zoneMaster.name as zoneName, siteMaster.name as siteName, sum(coalesce(collectionObj.amount, 0)) as totalAmount");
		queryStr.append(" from ApnimandiCollectionDetails collectionObj");
		queryStr.append(" inner join collectionObj.zone zoneMaster");
		queryStr.append(" inner join collectionObj.site siteMaster");
		queryStr.append(" inner join collectionObj.status egwStatus");
