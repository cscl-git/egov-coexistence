...
public class GeneralLedgerReport {
	...
	public LinkedList getGeneralLedgerList(final GeneralLedgerReportBean reportBean) throws TaskFailedException {
		...
		/*
		 * if (!cv.isValidCode(glCode1)) { LOGGER.error(glCode1 + " Not Valid"); throw
		 * taskExc; }
		 */
		...
		try {
			...
			String credit_receiptNo = "";
			String debit_receiptNo = "";
			/**
			 * When using ResultSet.TYPE_SCROLL_INSENSITIVE in createStatement if no records
			 * are there, rs.next() will return true but when trying to access
			 * (rs.getXXX()), it will throw an error
			 **/
			...
			if (resultset1 == null || resultset1.size() == 0) { // Will consider
				...
				final String arr[] = new String[17];
				...
				final String arr2[] = new String[17];
			...
			for (final Object[] element : resultset1) {
				...
				try {
					...
					if (isconfirmed != null && !isconfirmed.equalsIgnoreCase("")
				...
				if (curVHID > 0 && vhId != curVHID && cout == 0 && vhId != 0) {
					...
					if (txnDebit > 0) {
						previousDebit = 0;
						previousCredit = 0;
						final String arr9[] = new String[17];
						arr9[0] = vcDate;
						arr9[1] = vcNum;
						...
						arr9[12] = vcTypeName;
						arr9[13] = "";
						arr9[15] = debit_receiptNo;
						arr9[16] = credit_receiptNo;
						data.add(arr9);
					} else if (txnCredit > 0) {
						previousDebit = 0;
						previousCredit = 0;
				...
				if (!accEntityKey.equals(""))
					...
					System.out.println("accEntityKey---->" + accEntityKey);
				...
				if (accCode.equalsIgnoreCase(code)) {
					...
					/*
					 * if(slCrAmount.compareTo(BigDecimal.ZERO)!=0) { detail= detail.append(" " +
					 * glCode1+"&nbsp;&nbsp;&nbsp;"+ element[8].toString()); slCrAmount=new
					 * BigDecimal("0.00"); } else if(slDrAmount.compareTo(BigDecimal.ZERO)!=0) {
				...
				if (vhId != 0 && (detailId == null || detailId.equals(accEntityKey)) && !accEntityKey.equals("")) {
					...
					if (accCode.equalsIgnoreCase(code)) {
						 ...
						 * element[6].toString()+"&nbsp;&nbsp;&nbsp;"+ element[8].toString()); }
						 */
					} else if (vhId == VhidPrevious && accCode.equalsIgnoreCase(accCodePrevious) // &&
					// (StringUtils.isEmpty(reBean.getFunctionId())
					// ||
				...
				if (element.equals(resultset1.get(resultset1.size() - 1))) {
					...
					if (txnDebit > 0) {
						...
						arr[12] = vcTypeName;
						arr[13] = "";
						arr[15] = debit_receiptNo;
						arr[16] = credit_receiptNo;
					...
					data.add(arr2);
					final String arr1[] = new String[17];
					if (txnDrSum > 0)
					...
					arr1[15] = debit_receiptNo;
					arr1[16] = credit_receiptNo;
//					arr1[15] = "";
//					arr1[16] = "";
//					if(element[14] != null) {
//						final String sqlString1 = "select reciept_number  from vouchermis where id=?";
//						pstmt = persistenceService.getSession().createSQLQuery(sqlString1);
//						pstmt.setLong(0, Long.parseLong(arr1[14]));
//						final List res = pstmt.list();
//						res.toArray();
//					}
//					
...
//					}
			...
			final String gridData[][] = new String[data.size() + 1][17];
			...
			gridData[0][14] = "vhId";
			gridData[0][15] = "debit_receiptNo";
			gridData[0][16] = "credit_receiptNo";
			...
			for (int i = 1; i <= data.size(); i++) {
				...
				generalLedgerBean.setDebit_receiptNo(gridData[i][15]);
				generalLedgerBean.setCredit_receiptNo(gridData[i][16]);
	...
	private String getQuery(final String glCode1, final String startDate, final String endDate,
			...
			final String schemeId) throws TaskFailedException {
		...
		if (!accEntityKey.equals(""))
			return "SELECT  gl.glcode as \"code\",(select ca.type from chartofaccounts ca where glcode=gl.glcode) as \"glType\" ,"
					...
					+ " ELSE NULL END) as \"Name\",CASE WHEN gl.glcode = '" + glCode1
					...
					+ " AS \"type\"," + " CASE WHEN gl.glcode = '" + glCode1
					...
					+ " as \"debitamount\",CASE WHEN gl.glcode = '" + glCode1
					...
					+ " and cdet.glcodeid=coa.id " + " and gl.glcode='" + glCode1 + "'" + " AND f.id= vh.fundId "
					+ entityCondition + "" + " and vh.id in (" + engineQry + " )"
			...
			final StringBuffer sql = new StringBuffer(
					"SELECT  gl.glcode as \"code\",(select ca.type from chartofaccounts ca where glcode=gl.glcode) as \"glType\",vh.id AS \"vhid\", vh.voucherDate AS \"vDate\", ")
							...
							.append(" FROM generalLedger gl, voucherHeader vh, vouchermis vmis1, chartOfAccounts coa,  fund f "
									+ addTableToQuery + "")
							...
							.append(schemeCondition).append(" and vh.id in (" + engineQry + " )")
							.append(" group by vh.id,gl.glcode,vh.voucherDate ,vh.voucherNumber,coa.name,gl.description,vmis1.reciept_number,  vh.type || '-' || vh.name||CASE WHEN status = 1 THEN '(Reversed)' ELSE (CASE WHEN status = 2 THEN '(Reversal)' ELSE '' END) END, gl.debitamount , gl.creditamount  ,f.name, vh.isconfirmed, vh.type  ||'-'  ||vh.name, gl.functionid   ")
	...
	private OpBal getOpeningBalance(final String glCode, final String fundId, final String fundSourceId,
		...
		final String queryYearOpBal = "SELECT CASE WHEN sum(openingDebitBalance) is null THEN 0 ELSE sum(openingDebitBalance) END AS \"openingDebitBalance\", "
				...
				+ "FROM transactionSummary WHERE " + fundCondition + fundSourceCondition + functionCondition
				+ accEntityCondition + deptCondition + " financialYearId=? "
		...
		if (!accEntityId.equalsIgnoreCase("") && !accEntityKey.equalsIgnoreCase("")) {
			...
			queryTillDateOpBal = "SELECT coa.glcode,(SELECT SUM(gldet.amount) FROM generalLedger gl, voucherHeader vh "
					+ deptFromCondition + " , generalledgerdetail gldet "
					+ " WHERE vh.id = gl.voucherHeaderId AND gl.glcodeid IN (coa.id) " + fundCondition
					+ fundSourceCondition + functionCondition + accEntityCondition + " AND " + deptWhereCondition
					...
					+ deptFromCondition + " , " + " generalledgerdetail gldet WHERE vh.id = gl.voucherHeaderId AND "
					+ deptWhereCondition + " " + " gl.glcodeid IN (coa.id) " + fundCondition + fundSourceCondition
					+ functionCondition + accEntityCondition
			...
			queryTillDateOpBal = "SELECT CASE WHEN sum(gl.debitAmount) is null THEN 0 ELSE sum(gl.debitAmount) END AS \"debitAmount\", "
					...
					+ " FROM generalLedger gl, voucherHeader vh " + deptFromCondition
					+ " WHERE vh.id = gl.voucherHeaderId AND " + deptWhereCondition + " gl.glCode IN (?)" + " "
					+ fundCondition + fundSourceCondition + functionCondition
					+ " AND vh.voucherDate >= to_date(?,'dd/MM/YYYY') AND vh.voucherDate <to_date(?,'dd/MM/YYYY') AND vh.status not in ("
							...
							: Integer.parseInt(dt2[1]) > Integer.parseInt(dt1[1]) ? 1
									: Integer.parseInt(dt2[1]) < Integer.parseInt(dt1[1]) ? -1
											: Integer.parseInt(dt2[0]) > Integer.parseInt(dt1[0]) ? 1
													: Integer.parseInt(dt2[0]) < Integer.parseInt(dt1[0]) ? -1 : 0;
			if (ret == -1)
				throw taskExc;
