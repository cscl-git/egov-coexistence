...
function callAjaxSearch() {
	...
	jQuery('.report-section').removeClass('display-hide');
		reportdatatable = drillDowntableContainer
			.dataTable({
				ajax : {
					url : "/services/lcms/advocatemaster/ajaxsearch/"+$('#mode').val(),      
					type: "POST",
...
$("#resultTable").on('click','tbody tr',function(event) {
	window.open('/services/lcms/advocatemaster/'+ $('#mode').val() +'/'+drillDowntableContainer.fnGetData(this,3),'','width=800, height=600');
});
	 ...
	 if ($('#bankId').val() === '') {
		   ...
		   $('#bankBranch').append($('<option>').text('Select from below').attr('value', ''));
			return;
				...
				$.ajax({
					url: "/services/lcms/ajax-getAllBankBranchsByBank",    
