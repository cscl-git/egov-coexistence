	...
	public static ByteArrayInputStream estimateResultToExcel1(List<EstimatePreparationApproval> estimatelist,String[] COLUMNS)
		...
		try(
			...
			for (EstimatePreparationApproval estimate : estimatelist) {
				...
				row.createCell(0).setCellValue(estimate.getWorkName());
				row.createCell(1).setCellValue(estimate.getExecuteDiv());
				row.createCell(2).setCellValue(estimate.getWorksWing());
	            row.createCell(3).setCellValue(estimate.getExpHead_est());
	            row.createCell(4).setCellValue(estimate.getEstimateAmount());
	            row.createCell(5).setCellValue(estimate.getEstimateDt());
	            row.createCell(6).setCellValue(estimate.getCreatedDt());
			}
