$(document).ready(
	 ...
	 console.log("pass ::"+passUnderObjection);
			if (passUnderObjection == 0 || passUnderObjection == 'null'
					|| passUnderObjection == null) {

	 $('input[type=checkbox]').removeAttr('checked');
	 }
	
			if (passUnderObjection == 1) {
				myFunction();
			}
...
function openHistory(auditId,checkListId){
	var sourcepath = "/services/audit/createAudit/history/" + auditId + "/"
			+ checkListId;
	window
			.open(sourcepath, 'Source',
...
function downloadexcel(){
	...
	var url="/services/audit/createAudit/exportToExcel"+"?billFrom="+billFrom+"&billTo="+billTo+"&department="+department;
...
function openVoucher(vid) {
	var url = "/services/EGF/voucher/preApprovedVoucher-loadvoucherview.action?vhid="
			+ vid;
...
function updateAuditorsScreen(auditId) {
	...
	var url = "/services/audit/createAudit/updateAuditorScreen/" + auditId
			+ "/" + type;
	window.open(url,'','width=900, height=700');
}
...
$('#type')
	...
	var type =$('#type').val();
					...
					$
							.ajax({
								url : "/services/audit/createAudit/employee/"
										+ type,
type: "GET",
									...
									$
											.each(
													...
													function(index, value) {
														// $('#approvalPosition').append($('<option>').text(value.userName+'/'+value.positionName).attr('value',
														// value.positionId));
														$('#leadAuditorEmpNo')
																.append(
																		$(
...
function addcheckListRow(x) { 
	...
	if (rowcount < 30) {
		if (document.getElementById('tblchecklistRow') != null) {
			...
			$('#tblchecklist tbody tr:eq(' + rowcount + ')').find(
					'.checklist_description').val('');
			$('#tblchecklist tbody tr:eq(' + rowcount + ')').find('.status')
					.val('');
			$('#tblchecklist tbody tr:eq(' + rowcount + ')').find(
					'.checklist_date').val('');
			$('#tblchecklist tbody tr:eq(' + rowcount + ')').find(
			...
			addCustomEvent(rowcount, 'tempSubLedger[index].addButton',
					'keydown', shortKeyFunForAddButton);
		}
	} else {
...
function addcheckListRowNew() {
    ...
    element.attr('id', 'rec-'+size);
		element.find('.checklist_date').attr('id',
				"checkList[" + size + "].checklist_date");
    element.find('.checklist_description').val('');
    element.find('.status').attr('path', "checkList["+size+"].status");
    element.find('.status').val('');
		element.find('.checklist_date').attr('path',
				"checkList[" + size + "].checklist_date");
		// element.find('.checklist_date').attr('class', "form-control
		// datepicker checklist_date");
		element.find('.auditor_comment').attr('path',
				"checkList[" + size + "].auditor_commente");
    element.find('.auditor_comment').val('');
		element.find('.user_comments').attr('path',
				"checkList[" + size + "].user_comments");
...
function displayDate(size){
	...
	document.getElementById("checkList["+size+"].checklist_date").value=today;	
...
jQuery(document)
    ...
    if(rowcount<=1) {
						bootbox.alert($.i18n
								.prop('msg.this.row.can.not.be.deleted'));
		return false;
	}else{
   if(deleteurl[0] !== "0"){
							$
									.ajax({
										url : "/services/audit/createAudit/deleteAuditchecklist/"
												+ deleteurl[0],
    	type: "GET",
    	contentType:'application/json',
    	//data: JSON.stringify(jsonData),
    	success: function (response) {
    		console.log("success"+response);
											...
											jQuery('#rec-' + deleteurl[1])
													.remove();
											$('#confirm-deleterow').modal(
													'hide');
    	}, 
    	error: function (response) {
    		console.log("failed");
