...
<div class="panel panel-primary" data-collapsed="0" style=" scrollable:true;">
    ...
    <c:if test="${auditDetail.documentDetail != null &&  !auditDetail.documentDetail.isEmpty()}">
        <c:forEach items="${auditDetail.documentDetail }" var="documentDetials">
            <a href="/services/EGF/expensebill/downloadBillDoc?egBillRegisterId=${auditDetail.id }&fileStoreId=${auditDetail.fileStore.fileStoreId }">${documentDetials.fileStore.fileName }</a><br />
        </c:forEach>
    </c:if>
    <c:if test="${mode == 'view' && auditDetail.documentDetail.isEmpty()}">
