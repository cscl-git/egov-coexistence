...
<form:form name="auditForm" role="form" method="post" action="save"
	modelAttribute="auditDetail" id="auditDetail"
			...
			<div class="panel panel-primary" data-collapsed="0">
				<div class="form-group" style="padding: 50px 20px 0;">
					<label class="col-sm-3 control-label text-left-audit"><spring:message
							code="lbl.auditnumber" text="Audit Number" /> <span
						class="mandatory"></span> </label>
					<div class="col-sm-3 add-margin">
						<form:input class="form-control patternvalidation"
							data-pattern="alphanumericwithspecialcharacters" id="auditnumber"
							path="auditNumber" maxlength="50" required="required"
							readonly="true" />
						<form:errors path="auditNumber" cssClass="add-margin error-msg" />
					</div>

					<label class="col-sm-3 control-label text-left-audit"><spring:message
							code="lbl.schdauditdate" text="Audit Scheduled Date" /> <span
						class="mandatory"></span> </label>
					<div class="col-sm-3 add-margin">
						<c:if test="${auditDetail.auditStatus == 'Created' }">
							<form:input id="auditScheduledDate" path="auditScheduledDate"
								class="form-control datepicker" data-date-end-date="0d"
								required="required" placeholder="DD/MM/YYYY" />
							<form:errors path="auditScheduledDate"
								cssClass="add-margin error-msg" />
						</c:if>
						<c:if
							test="${auditDetail.auditStatus == 'Pending with Department'  || auditDetail.auditStatus =='Pending with Auditor' || auditDetail.auditStatus =='Pending with Section Officer'  || auditDetail.auditStatus =='Pending with Examiner'}">
							<form:input id="auditScheduledDate" path="auditScheduledDate"
								class="form-control datepicker" data-date-end-date="0d"
					...
					<div class="col-sm-3 add-margin">
						<form:input class="form-control patternvalidation"
							...
							path="auditType" readonly="true" />
					</div>
					<c:if test="${auditDetail.auditType == 'Pre-Audit' }">
						<label class="col-sm-3 control-label text-left-audit"><spring:message
								code="lbl.billDetails" text="Bill Details" /> </label>
						<div class="col-sm-3 add-margin">
