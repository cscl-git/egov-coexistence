...
function callAjaxSearch() {
	...
	jQuery('.report-section').removeClass('display-hide');
		reportdatatable = drillDowntableContainer
			.dataTable({
				ajax : {
					url : "/services/council/councilmember/ajaxsearch/"+$('#mode').val(),      
					type: "POST",
				...
				console.log(data.id);
				window.open('/services/council/councilmember/'+ $('#mode').val() +'/'+data.id,'','width=800, height=600,scrollbars=yes');
			});
				 },*/
				"bDestroy" : true,
				"autoWidth": false,
...
$("#resultTable").on('click','tbody tr td  .view',function(event) {
	...
	window.open('/services/council/councilmember/'+ $('#mode').val() +'/'+id,'','width=800, height=600,scrollbars=yes');
